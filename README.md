...
As long as our choice of points is random we might have to loop through all possible triples in the worst case. But random walk is not the best strategy for finding the triple we're interested in that satisfies our criteria, there's a better algorithm for that ) The goal is to find the best three points that enclose the Origin out of all possible triples of points, if such a combination is possible at all.
