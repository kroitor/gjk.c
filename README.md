...
The logic behind this can be described in the following way: imagine we *stand at* point `C` and look towards the Origin from that perspective. We want to find next point that would be *beyond* the Origin as seen by us. If there's no such point then we will not be able to surround the Origin with a triangle using current point `C`. If we cannot reach for a point beyond the Origin from current standpoint `C`, then there there's probably some distance between initial shapes (at least those two shapes don't intersect in that dimension). So, no collision is detected on this iteration and we probably need to restart another iteration of evolution with a different starting point (different initial direction `D`). The halting criteria of GJK algorithm says that current iteration stops if our current simplex fails to include (or surround) the Origin, handling this will be explained in more detail later in this text.
